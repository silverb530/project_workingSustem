...
def _reload_employees():
    try:
        from FaceReco.recognize import load_employees
        load_employees()
        print("[INFO] 얼굴 임베딩 DB 재로드 완료")
    except Exception as e:
        print(f"[WARN] 얼굴 임베딩 재로드 실패: {e}")
...
def generate_register_frames():
    while True:
        ...
        label = f"Capturing: {cap_state['count']}/{CAPTURE_MAX}" if cap_state['capturing'] else "Ready"
