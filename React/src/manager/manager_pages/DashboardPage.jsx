...
function TaskSection() {
  ...
  const StatusIcon = ({ status }) => {
    switch (status) {
      ...
      case 'in-progress': return <Icons.Clock />
      ...
      <div className="card-content">
        {showForm && (
          <div className="meeting-create-form">
            <input className="admin-input" placeholder="업무 제목" value={title} onChange={(e) => setTitle(e.target.value)} />
            <input className="admin-input" placeholder="담당 부서" value={department} onChange={(e) => setDepartment(e.target.value)} />
            <input className="admin-input" placeholder="담당자" value={assignee} onChange={(e) => setAssignee(e.target.value)} />
            <input className="admin-input" placeholder="마감일" value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
            <div className="meeting-create-actions">
              <button className="btn btn-primary btn-sm" onClick={handleCreateTask}>저장</button>
              <button className="btn btn-outline btn-sm" onClick={() => setShowForm(false)}>취소</button>
            </div>
          </div>
        )}
        ...
        <div className="task-list">
          {tasks.map((task) => (
            <div key={task.id} className={`task-item ${task.status === 'done' ? 'completed' : ''}`}>
              <button className={`task-status-btn ${task.status}`}>
              ...
              <div className="task-meta">
                ...
                <button className="btn btn-destructive btn-sm" onClick={() => handleDeleteTask(task.id)}>
                  삭제
                </button>
              </div>
            </div>
...
function MeetingSection() {
  ...
  async function handleJoinMeeting(meetingId) {
    try {
      setError('')
      setMessage('')
      await apiPost(`/api/meetings/${meetingId}/join`, { user: 'manager' })
      setMessage('회의 참가 완료')
      await loadMeetings()
    } catch (err) {
      setError(err.message)
