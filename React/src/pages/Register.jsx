...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const success = res.data.success === true || res.data.result === 'success';

            if (success) {
                const employeeId = res.data.employee_id;
                const qrData = res.data.qr_data || '';
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div style={styles.row}>
                        <Field
                            ...
                            value={form.pc_name}
                            onChange={handleChange}
                            placeholder="미입력 시 이름+의 PC"
                        />

                        <Field
                            label={ipLoading ? "PC IP 주소 * (감지 중...)" : "PC IP 주소 * (자동 감지)"}
                            name="ip_address"
                            value={form.ip_address}
                            onChange={handleChange}
                            placeholder="예) 192.168.0.101"
                        />
                    </div>
